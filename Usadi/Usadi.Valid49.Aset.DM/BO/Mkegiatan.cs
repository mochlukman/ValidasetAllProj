...
{
  #region Usadi.Valid49.BO.MkegiatanControl, Usadi.Valid49.Aset.DM
  [Serializable]
  public class MkegiatanControl : BaseDataControlAsetDM, IDataControlUIEntry,IDataControlTreeGrid3, IHasJSScript
  {
    #region Properties
    ...
    #region Methods
    public MkegiatanControl()
    {
      XMLName = ConstantTablesAsetDM.XMLMKEGIATAN;
    }
    public new IProperties GetProperties()
    {
      ...
      cViewListProperties.IDKey = "Kdkegunit";//IDKey for ID Notes
      cViewListProperties.IDProperty = "Kdkegunit";//UniqueKey in gridview
    ...
    {
      if (typeof(MpgrmControl).IsInstanceOfType(bo))
      {
        Uruskey = ((MpgrmControl)bo).Uruskey;
        Kdurus = ((MpgrmControl)bo).Kdurus;
        Nmurus = ((MpgrmControl)bo).Nmurus;
    ...
    public new HashTableofParameterRow GetFilters()
    {
      ...
      //hpars.Add(MpgrmLookupControl.Instance.GetLookupParameterRow(this, true).SetAllowRefresh(false).SetEnable(enableFilter));
      return hpars;
    }
    ...
    public new IList View(string label)
    {
      ...
      List<MkegiatanControl> ListData = new List<MkegiatanControl>();
      foreach (MkegiatanControl dc in list)
      {
        ListData.Add(dc);
      }
      //Update(ListData);
    ...
    //  return ListData;
    //}
    public static bool IsRootCondition(MkegiatanControl dc)
    {
      return (dc.Kdlevel == 1);
    }
    public Icon GetIcon()
    ...
    public Ext.Net.TreeNode CreateRoot(IList list, int typetree, bool withroot)
    {
      ...
      List<MkegiatanControl> localList = (List<MkegiatanControl>)list;
      List<MkegiatanControl> roots = localList.FindAll(i => IsRootCondition(i));

      Ext.Net.TreeNode root = new Ext.Net.TreeNode("Root", "Root", GetIcon());
      Ext.Net.TreeNodeCollection nodes = root.Nodes;
      foreach (MkegiatanControl ctrl in roots)
      {
        nodes.Add(CreateNodeWithOutChildren((List<MkegiatanControl>)list, ctrl, typetree, delim_menu));
      }
      return root;
    }
    ...
    {
      string delim_menu = ">";
      List<MkegiatanControl> list = (List<MkegiatanControl>)inList;
      if (list != null && list.Count > 0)
      {
        MkegiatanControl parent = list.Find(o => o.Nukeg.Equals(nodeid));
        if ((parent != null) && (parent.Type.Equals("H")))
        {
          List<MkegiatanControl> children = GetChildren(list, parent);
          foreach (MkegiatanControl ctrl in children)
          {
    ...
    public static List<MkegiatanControl> GetChildren(List<MkegiatanControl> domainset, MkegiatanControl parent)
    {
      List<MkegiatanControl> children = domainset.FindAll(o => o.Nukeg.StartsWith(parent.Nukeg) && (o.Kdlevel == parent.Kdlevel + 1));
      return children;
    }
    protected TreeNodeBase CreateNodeWithOutChildren(List<MkegiatanControl> list, MkegiatanControl parent, int typetree, string delim_menu)
    {
    ...
    #region Sum All
    public static void Update(List<MkegiatanControl> domainset)
    {
      List<MkegiatanControl> roots = domainset.FindAll(i => IsRootCondition(i));
      if (roots.Count > 0)
      {
        foreach (MkegiatanControl r in roots)
        {
    ...
    static UpdatedRecFields SumChildren(List<MkegiatanControl> domainsetParent, MkegiatanControl parent)
    {
      UpdatedRecFields localBobotProgress = new UpdatedRecFields();
      List<MkegiatanControl> localDomainset = domainsetParent.FindAll(o => o.Nukeg.StartsWith(parent.Nukeg));

      List<MkegiatanControl> children = GetChildren(localDomainset, parent);
      int localNumChildren = 0;
      if ((children.Count > 0) && (parent.Type == "H"))
      {
        localBobotProgress.Nchild = children.Count;
        foreach (MkegiatanControl o in children)
        {
          localNumChildren++;
    ...
    class UpdatedRecFields
    {
      ...
      public void SetValue(MkegiatanControl rec)
      {
        Nchild = rec.Nchild;
      }
    }
