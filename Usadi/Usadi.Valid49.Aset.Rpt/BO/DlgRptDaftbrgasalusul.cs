...
{
  #region Usadi.Valid49.BO.DlgRptDaftbrgasalusulControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptDaftbrgasalusulControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptDaftbrgasalusulControl()
    {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      hpars.Add(new ParameterRowSelect(ConstantDict.GetColumnTitle("Kdklas=Kelas Aset"),
        GetList(new JklasRptLookupControl()), "Kdklas=Uraiklas", 54).SetEnable(true));
      ...
      hpars.Add(new ParameterRow(ConstantDict.GetColumnTitleEntry("Subunit=Subunit"), ParameterRow.MODE_SELECT,
        list, "Kdpar=Nmpar", 26).SetEnable(true));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "DaftBrgAsalUsul.rpt";
      CS[ReportUtils.PDFNAME] = "DaftBrgAsalUsul_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      string pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.DaftBrgAsalUsul.rpt", CS, Params);
      return pdfurl;
    }
    ...
    {
      RptPars rptPar = new RptPars() { Title = "DAFT BRG ASAL USUL", RptClass = LinkPdf };
  ...
  #endregion DlgRptDaftbrgasalusul
}
