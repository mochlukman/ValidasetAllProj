...
{
  #region Usadi.Valid49.BO.DlgRptRincpenguranganasettetapintraControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRincpenguranganasettetapintraControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptRincpenguranganasettetapintraControl()
    {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      hpars.Add(new ParameterRow(ConstantDict.GetColumnTitleEntry("StrMode=Pengurangan ke"), ParameterRow.MODE_SELECT,
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = $"RINCPENGURANGAN_ASETTETAPINTRA_{StrMode}.rpt";
      CS[ReportUtils.PDFNAME] = $"RINCPENGURANGAN_ASETTETAPINTRA_{StrMode}_{(string)GlobalAsp.GetSessionUser().GetValue("Userid")}_{ DateTime.Now.ToString("yyyyMMddHHmmss")}.pdf";
      ...
      //default,Aset tetap,Aset tetap ekstra,Aset lancar,Aset lainnya,Alasan lainnya&
      //maping suffix lowcase
      MapMode["default"] = string.Empty;
      MapMode["aset tetap"] = "ASETTETAP";
      MapMode["aset tetap ekstra"] = "ASETTETAPEKSTRA";
      MapMode["aset lancar"] = "ASETLANCAR";
      MapMode["aset lainnya"] = "ASETLAINNYA";
      MapMode["alasan lainnya"] = "ALASANLAIN";

      string cname = "Usadi.Valid49.Aset.Rpt.Rpt.RINCPENGURANGAN_ASETTETAPINTRA_ASETTETAP.rpt";
      if (!StrMode.Equals("default"))
      {
        //string strmode = StrMode.Replace(" ", "");
        string strmode = MapMode[StrMode.ToLower()];
        cname = $"Usadi.Valid49.Aset.Rpt.Rpt.RINCPENGURANGAN_ASETTETAPINTRA_{strmode}.rpt";
      }
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF(cname, CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = $"RINCPENGURANGAN_ASETTETAPINTRA_{StrMode}_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportPDF(cname, CS, Params);
      }
      return pdfurl;
  ...
  #endregion DlgRptRincpenguranganasettetapintra
}
