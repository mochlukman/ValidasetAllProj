...
{
  #region Usadi.Valid49.BO.DlgRptRekappenyusutanControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRekappenyusutanControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "REKAPPENYUSUTAN.rpt";
      CS[ReportUtils.PDFNAME] = "REKAPPENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.REKAPPENYUSUTAN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "REKAPPENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.REKAPPENYUSUTAN.rpt", CS, Params);
      }
      return pdfurl;
    }
