...
{
  #region Usadi.Valid49.BO.DlgRptRekaplapmutasiControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRekaplapmutasibrgControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptRekaplapmutasibrgControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "REKAPLAPMUTASI.rpt";
      CS[ReportUtils.PDFNAME] = "REKAPLAPMUTASI_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.REKAPLAPMUTASI.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "REKAPLAPMUTASI_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.REKAPLAPMUTASI.rpt", CS, Params);
      } 
      return pdfurl;
    }
  ...
  #endregion DlgRptRekaplapmutasibrg
}
