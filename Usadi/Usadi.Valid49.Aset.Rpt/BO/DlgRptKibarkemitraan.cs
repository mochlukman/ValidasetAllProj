...
{
  #region Usadi.Valid49.BO.DlgRptKibarkemitraanControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptKibarkemitraanControl : KibkemitraanControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptKibarkemitraanControl()
    {
      XMLName = ConstantTablesAsetMAT.KDKIBKEMITRAAN;
    }
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "KIBARKEMITRAAN.rpt";
      CS[ReportUtils.PDFNAME] = "KIBARKEMITRAAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.KIBARKEMITRAAN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "KIBARKEMITRAAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.KIBARKEMITRAAN.rpt", CS, Params);
      }
      return pdfurl;
    }
