...
{
  #region Usadi.Valid49.BO.DlgRptDaftpeliharabrgControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptDaftpeliharabrgControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    #endregion
    public DlgRptDaftpeliharabrgControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "DAFTPELIHARABRG.rpt";
      CS[ReportUtils.PDFNAME] = "DAFTPELIHARABRG_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.DAFTPELIHARABRG.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "DAFTPELIHARABRG_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.DAFTPELIHARABRG.rpt", CS, Params);
      }
      return pdfurl;
    }
