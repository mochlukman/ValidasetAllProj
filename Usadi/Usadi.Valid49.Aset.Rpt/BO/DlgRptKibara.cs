...
{
  #region Usadi.Valid49.BO.DlgRptKibaraControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptKibaraControl : KibaControl, IDataControlUIEntry, IPrintable
  {
    ...
    public new HashTableofParameterRow GetFilters()
    {
      ...
      //hpars.Add(TahunLookupControl.Instance.GetLookupParameterRow(this, false).SetAllowRefresh(true).SetEnable(enableFilter).SetAllowEmpty(false));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "KIBARA.rpt";
      CS[ReportUtils.PDFNAME] = "KIBARA_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.KIBARA.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "KIBARA_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.KIBARA.rpt", CS, Params);
      }
      return pdfurl;
    }
