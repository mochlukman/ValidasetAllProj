...
{
  #region Usadi.Valid49.BO.DlgRptDafthapusControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptDafthapusControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      hpars.Add(new ParameterRowSelect(ConstantDict.GetColumnTitle("Kdtans=Jenis Penghapusan"),
        GetList(new JtransRptLookupControl()), "Kdtans=Nmtrans", 54).SetEnable(enableFilter));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "DaftBrgHapus.rpt";
      CS[ReportUtils.PDFNAME] = "DaftBrgHapus_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";

      Hashtable Params = new Hashtable();
      Params["@unitkey"] = Unitkey;
      Params["@kdhapus"] = Kdtans;
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.DaftBrgHapus.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "DaftBrgHapus_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.DaftBrgHapus.rpt", CS, Params);
      } 
      return pdfurl;
    }
