...
{
  #region Usadi.Valid49.BO.DlgRptPengakhiranppenggunaansementaraControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptPengakhiranppenggunaansementaraControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string StrMode { get; set; } = "Sementara";
    public DlgRptPengakhiranppenggunaansementaraControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = $"PENGAKHIRANPENGGUNAAN_{StrMode}.rpt";
      CS[ReportUtils.PDFNAME] = $"PENGAKHIRANPENGGUNAAN_{StrMode}_{(string)GlobalAsp.GetSessionUser().GetValue("Userid")}_{ DateTime.Now.ToString("yyyyMMddHHmmss")}.pdf";
      ...
      string cname = "Usadi.Valid49.Aset.Rpt.Rpt.PENGAKHIRANPENGGUNAAN_SEMENTARA.rpt";
      if (!StrMode.Equals("default"))
      {
        //string strmode = StrMode.Replace(" ", "");
        string strmode = MapMode[StrMode.ToLower()];
        cname = $"Usadi.Valid49.Aset.Rpt.Rpt.PENGAKHIRANPENGGUNAAN_{strmode}.rpt";
      }
    ...
    {
      RptPars rptPar = new RptPars() { Title = "Laporan Pengakhiran Penggunaan", RptClass = LinkPdf };
  ...
  #endregion DlgRptPengakhiranppenggunaansementara
}
