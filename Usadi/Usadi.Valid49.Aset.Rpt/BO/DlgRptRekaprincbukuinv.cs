...
{
  #region Usadi.Valid49.BO.DlgRptRekaprincbukuinvControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRekaprincbukuinvControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptRekaprincbukuinvControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "REKAPRINCBI.rpt";
      CS[ReportUtils.PDFNAME] = "REKAPRINCBI_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.REKAPRINCBI.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "REKAPRINCBI_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.REKAPRINCBI.rpt", CS, Params);
      } 
      return pdfurl;
    }
  ...
  #endregion DlgRptRekaprincbukuinv
}
