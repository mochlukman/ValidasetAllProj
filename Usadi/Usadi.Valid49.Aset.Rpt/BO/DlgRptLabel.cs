...
{
  #region Usadi.Valid49.BO.DlgRptLabelControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptLabelControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "Label.rpt";
      CS[ReportUtils.PDFNAME] = "Label_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.Label.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "Label_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.Label.rpt", CS, Params);
      }
      return pdfurl;
    }
