  ...
  public class DlgRptRekapBukuInvControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      hpars.Add(new ParameterRow(ConstantDict.GetColumnTitleEntry("Subunit=Subunit"), ParameterRow.MODE_SELECT,
        list, "Kdpar=Nmpar", 26).SetAllowRefresh(false).SetEnable(enableFilter));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "RekapBukuInv.rpt";
      CS[ReportUtils.PDFNAME] = "RekapBukuInv_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.RekapBukuInv.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "RekapBukuInv_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.RekapBukuInv.rpt", CS, Params);
      } 
      return pdfurl;
    }
