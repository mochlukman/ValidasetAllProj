...
namespace Usadi.Valid49.BO
{
  ...
  public class DlgRptDaftpengadaanControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptDaftpengadaanControl()
    {
      ...
      Tglawal = new DateTime((Int32.Parse(cPemda.Configval)), 01, 01);
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.PDFNAME] = "Daftpengadaan_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.Daftpengadaan.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "Daftpengadaan_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.Daftpengadaan.rpt", CS, Params);
      } 
      return pdfurl;
    }
