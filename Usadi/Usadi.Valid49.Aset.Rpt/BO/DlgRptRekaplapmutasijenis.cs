...
{
  #region Usadi.Valid49.BO.DlgRptRekaplapmutasijenisControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRekaplapmutasijenisControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptRekaplapmutasijenisControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "REKAPLAPMUTASI_JENIS.rpt";
      CS[ReportUtils.PDFNAME] = "REKAPLAPMUTASI_JENIS_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.REKAPLAPMUTASI_JENIS.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "REKAPLAPMUTASI_JENIS_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.REKAPLAPMUTASI_JENIS.rpt", CS, Params);
      } 
      return pdfurl;
    }
  ...
  #endregion DlgRptRekaplapmutasijenis
}
