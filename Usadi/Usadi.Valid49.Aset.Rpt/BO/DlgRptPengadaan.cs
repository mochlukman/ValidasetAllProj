...
{
  #region Usadi.Valid49.BO.DlgRptPengadaanControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptPengadaanControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      hpars.Add(JnskibFilterLookupControl.Instance.GetLookupParameterRow(this, false).SetAllowRefresh(true).SetAllowEmpty(false));
      //hpars.Add(new ParameterRowSelect(ConstantDict.GetColumnTitle("Kdkib=Jenis Aset"),
      //  GetList(new JnskibLookupControl()), "Kdkib=Nmkib", 54).SetEnable(enableFilter));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "PENGADAAN.rpt";
      CS[ReportUtils.PDFNAME] = "PENGADAAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.PENGADAAN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "PENGADAAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.PENGADAAN.rpt", CS, Params);
      }
