...
{
  #region Usadi.Valid49.BO.DlgRptReklasATBControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptReklasATBControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "RINCREKLAS_ATB.rpt";
      CS[ReportUtils.PDFNAME] = "RINCREKLAS_ATB_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.RINCREKLAS_ATB.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "RINCREKLAS_ATB_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.RINCREKLAS_ATB.rpt", CS, Params);
      } 
      return pdfurl;
    }
