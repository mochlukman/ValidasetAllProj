...
{
  #region Usadi.Valid49.BO.DlgRptLapbmdControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptLapbmdControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "LAPBMD.rpt";
      CS[ReportUtils.PDFNAME] = "LAPBMD_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.LAPBMD.rpt", CS, Params);
      }
      else
      {
