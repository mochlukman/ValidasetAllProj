...
{
  #region Usadi.Valid49.BO.DlgRptPenyusutanControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptPenyusutanControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptPenyusutanControl()
    {
  ...
  GetList(new DmbulanLookupControl()), "Bulan=Nmbulan", 26).SetEnable(enableFilter));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "PENYUSUTAN.rpt";
      CS[ReportUtils.PDFNAME] = "PENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if(n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.PENYUSUTAN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "PENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.PENYUSUTAN.rpt", CS, Params);
      } 
      return pdfurl;
    }
