...
{
  #region Usadi.Valid49.BO.DlgRptAsetlainlainControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptAsetlainlainControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptAsetlainlainControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "ASETLAIN_LAIN.rpt";
      CS[ReportUtils.PDFNAME] = "ASETLAINLAIN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.ASETLAIN_LAIN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "ASETLAIN_LAIN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.ASETLAIN_LAIN.rpt", CS, Params);
      }
      return pdfurl;
    }
