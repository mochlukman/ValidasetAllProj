...
{
  #region Usadi.Valid49.BO.DlgRptPenggunaanpspControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptPenggunaanpspControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptPenggunaanpspControl()
    {
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = $"PENGGUNAAN_{StrMode}.rpt";
      CS[ReportUtils.PDFNAME] = $"PENGGUNAAN_{StrMode}_{(string)GlobalAsp.GetSessionUser().GetValue("Userid")}_{ DateTime.Now.ToString("yyyyMMddHHmmss")}.pdf";


      Hashtable Params = new Hashtable();
      Params["@unitkey"] = Unitkey;
      //Params["@Jtrans"] = Kdtans;
      ...
      MapMode["pengalihan status"] = "PSP";
      MapMode["sementara"] = "SEMENTARA";
      MapMode["pihak lain"] = "DIOPERASIKANPIHAKLAIN";

      string cname = "Usadi.Valid49.Aset.Rpt.Rpt.PENGGUNAAN_PSP.rpt";
      if (!StrMode.Equals("default"))
      {
        //string strmode = StrMode.Replace(" ", "");
        string strmode = MapMode[StrMode.ToLower()];
        cname = $"Usadi.Valid49.Aset.Rpt.Rpt.PENGGUNAAN_{strmode}.rpt";
      }
    ...
    {
      RptPars rptPar = new RptPars() { Title = "Laporan Penerimaan Penggunaan ", RptClass = LinkPdf };
