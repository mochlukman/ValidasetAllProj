...
{
  #region Usadi.Valid49.BO.DlgRptRekapakumpenyusutanControl, Usadi.Valid49.Aset.Rpt
  [Serializable]
  public class DlgRptRekapakumpenyusutanControl : DaftunitControl, IDataControlUIEntry, IPrintable
  {
    ...
    public DlgRptRekapakumpenyusutanControl()
    {
    ...
    public override HashTableofParameterRow GetEntries()
    {
      ...
      //  GetList(new BulanLookupControl()), "Kd_bulan=Ket_bulan", 26).SetEnable(enableFilter));
      ...
      hpars.Add(new ParameterRowSelect(ConstantDict.GetColumnTitle("Bulan"),
        GetList(new DmbulanLookupControl()), "Bulan=Nmbulan", 26).SetEnable(enableFilter));
      hpars.Add(new ParameterRowSelect(ConstantDict.GetColumnTitle("Kdtahun=Tahun"),
        GetList(new TahunLookupControl()), "Kdtahun=Nmtahun", 26).SetEnable(enableFilter));
    ...
    public string GetURLReport(int n)
    {
      ...
      CS[ReportUtils.RPTNAME] = "REKAPAKUMPENYUSUTAN.rpt";
      CS[ReportUtils.PDFNAME] = "REKAPAKUMPENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
      ...
      if (n == 0)
      {
        pdfurl = ReportUtils.ExportPDF("Usadi.Valid49.Aset.Rpt.Rpt.REKAPAKUMPENYUSUTAN.rpt", CS, Params);
      }
      else
      {
        CS[ReportUtils.PDFNAME] = "REKAPAKUMPENYUSUTAN_" + (string)GlobalAsp.GetSessionUser().GetValue("Userid") + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
        pdfurl = ReportUtils.ExportExcel("Usadi.Valid49.Aset.Rpt.Rpt.REKAPAKUMPENYUSUTAN.rpt", CS, Params);
      }
      return pdfurl;
    }
