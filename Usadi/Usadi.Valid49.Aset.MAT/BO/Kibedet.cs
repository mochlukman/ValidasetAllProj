...
{
    #region Usadi.Valid49.BO.KibedetControl, Usadi.Valid49.Aset.MAT
    [Serializable]
    public class KibedetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript, IExtDataControl
    {
        #region Properties
        ...
        #region Methods 
        public KibedetControl()
        {
            XMLName = ConstantTablesAsetMAT.XMLKIBEDET;
        }
        ...
        {
            if (typeof(KibeControl).IsInstanceOfType(bo))
            {
                Idbrg = ((KibeControl)bo).Idbrg;
                Unitkey = ((KibeControl)bo).Unitkey;
                Asetkey = ((KibeControl)bo).Asetkey;
                Noreg = ((KibeControl)bo).Noreg;
            }
        }
        ...
        public new IList View(string label)
        {
            ...
            List<KibedetControl> ListData = new List<KibedetControl>();
            foreach (KibedetControl dc in list)
            {
                ListData.Add(dc);
            }
        ...
        public void SetTotal(Control seed)
        {
            if (true)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    {
                        KibedetControl ctrl = (KibedetControl)list[i];
                        if ((i >= start) && (i <= finish))
                        {
