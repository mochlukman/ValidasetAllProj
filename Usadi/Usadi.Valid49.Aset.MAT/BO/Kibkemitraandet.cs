...
{
  #region Usadi.Valid49.BO.KibkemitraandetControl, Usadi.Valid49.Aset.MAT
  [Serializable]
  public class KibkemitraandetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript, IExtDataControl
  {
    #region Properties
    ...
    #region Methods 
    public KibkemitraandetControl()
    {
      XMLName = ConstantTablesAsetMAT.XMLKIBKEMITRAANDET;
    }
    ...
    {
      if (typeof(KibkemitraanControl).IsInstanceOfType(bo))
      {
        Idbrg = ((KibkemitraanControl)bo).Idbrg;
        Unitkey = ((KibkemitraanControl)bo).Unitkey;
        Asetkey = ((KibkemitraanControl)bo).Asetkey;
      }
    }
    ...
    public new IList View(string label)
    {
      ...
      List<KibkemitraandetControl> ListData = new List<KibkemitraandetControl>();
      foreach (KibkemitraandetControl dc in list)
      {
        ListData.Add(dc);
      }
    ...
    public void SetTotal(Control seed)
    {
      if (true)
      {
        ...
        if (list != null && list.Count > 0)
        {
          ...
          {
            KibkemitraandetControl ctrl = (KibkemitraandetControl)list[i];
            if ((i >= start) && (i <= finish))
            {
