...
{
    #region Usadi.Valid49.BO.KibcdetControl, Usadi.Valid49.Aset.MAT
    [Serializable]
    public class KibcdetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript, IExtDataControl
    {
        #region Properties
        ...
        #region Methods 
        public KibcdetControl()
        {
            XMLName = ConstantTablesAsetMAT.XMLKIBCDET;
        }
        ...
        {
            if (typeof(KibcControl).IsInstanceOfType(bo))
            {
                Idbrg = ((KibcControl)bo).Idbrg;
                Unitkey = ((KibcControl)bo).Unitkey;
                Asetkey = ((KibcControl)bo).Asetkey;
                Noreg = ((KibcControl)bo).Noreg;
            }
        }
        ...
        public new IList View(string label)
        {
            ...
            List<KibcdetControl> ListData = new List<KibcdetControl>();
            foreach (KibcdetControl dc in list)
            {
                ListData.Add(dc);
            }
        ...
        public void SetTotal(Control seed)
        {
            if (true)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    {
                        KibcdetControl ctrl = (KibcdetControl)list[i];
                        if ((i >= start) && (i <= finish))
                        {
