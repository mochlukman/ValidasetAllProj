...
{
    #region Usadi.Valid49.BO.KibgdetControl, Usadi.Valid49.Aset.MAT
    [Serializable]
    public class KibgdetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript, IExtDataControl
    {
        #region Properties
        ...
        #region Methods 
        public KibgdetControl()
        {
            XMLName = ConstantTablesAsetMAT.XMLKIBGDET;
        }
        ...
        {
            if (typeof(KibgControl).IsInstanceOfType(bo))
            {
                Idbrg = ((KibgControl)bo).Idbrg;
                Unitkey = ((KibgControl)bo).Unitkey;
                Asetkey = ((KibgControl)bo).Asetkey;
                Noreg = ((KibgControl)bo).Noreg;
            }
        }
        ...
        public new IList View(string label)
        {
            ...
            List<KibgdetControl> ListData = new List<KibgdetControl>();
            foreach (KibgdetControl dc in list)
            {
                ListData.Add(dc);
            }
        ...
        public void SetTotal(Control seed)
        {
            if (true)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    {
                        KibgdetControl ctrl = (KibgdetControl)list[i];
                        if ((i >= start) && (i <= finish))
                        {
