...
{
    #region Usadi.Valid49.BO.KibddetControl, Usadi.Valid49.Aset.MAT
    [Serializable]
    public class KibddetControl : BaseDataControlAsetMAT, IDataControlUIEntry, IHasJSScript, IExtDataControl
    {
        #region Properties
        ...
        #region Methods 
        public KibddetControl()
        {
            XMLName = ConstantTablesAsetMAT.XMLKIBDDET;
        }
        ...
        {
            if (typeof(KibdControl).IsInstanceOfType(bo))
            {
                Idbrg = ((KibdControl)bo).Idbrg;
                Unitkey = ((KibdControl)bo).Unitkey;
                Asetkey = ((KibdControl)bo).Asetkey;
                Noreg = ((KibdControl)bo).Noreg;
            }
        }
        ...
        public new IList View(string label)
        {
            ...
            List<KibddetControl> ListData = new List<KibddetControl>();
            foreach (KibddetControl dc in list)
            {
                ListData.Add(dc);
            }
        ...
        public void SetTotal(Control seed)
        {
            if (true)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    {
                        KibddetControl ctrl = (KibddetControl)list[i];
                        if ((i >= start) && (i <= finish))
                        {
